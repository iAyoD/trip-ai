...
const moveAttraction = (dayIndex: number, attrIndex: number, direction: 'up' | 'down') => {
  ...
  if (direction === 'up' && attrIndex > 0) {
    const current = attractions[attrIndex];
    const prev = attractions[attrIndex - 1];
    if (current && prev) {
        attractions[attrIndex] = prev;
        attractions[attrIndex - 1] = current;
    }
  } else if (direction === 'down' && attrIndex < attractions.length - 1) {
    const current = attractions[attrIndex];
    const next = attractions[attrIndex + 1];
    if (current && next) {
        attractions[attrIndex] = next;
        attractions[attrIndex + 1] = current;
    }
  }
}
...
const getAttractionImage = (name: string, index: number): string => {
  ...
  if (!colorObj) return '' // Should not happen
  const { start, end } = colorObj
